...
It is important to note that the circuits shown in @fig-iq-divider has to be implemented in a way so that there is no phase ambiguity, i.e., the I-phase is lagging the Q-phase by exactly 90°. This can be achieved by using a reset signal to set the flip-flops to a known state at startup, or implement flip-flips using a differential clock input.
...
Often, we need 4 four-phase LO signal with 25% duty cycle (instead of the 50% duty cycle shown in @fig-ff-phase-diagram). This can be achieved by using additional logic gates to combine the outputs of the flip-flops, as shown in @fig-logen-25. The resulting LO waveforms are shown in @fig-logen-25-waveforms.
...
With a phase detector (PD) we can compare the phase of the output signal with the input signal, and regulate the delay per stage so that after a delays the edges line up. Then we can tap the equally-spaced phases and use them a LO signals. The advantage of this approach is that we can flexibly create a required number of phases from an input clock. This feature is also used in wireline communication system to generate multiple sampling phases for the data sampling.
