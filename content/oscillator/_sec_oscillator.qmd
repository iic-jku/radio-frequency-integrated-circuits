...
At first glance @eq-leeson-equation seems to say that a *large* tank resistance $R_\mathrm{p}$ is bad, since it sits in the numerator. This is misleading, because $Q$ in the denominator itself depends on $R_\mathrm{p}$. Substituting $Q = \omega_0 C R_\mathrm{p}$ makes the true dependence explicit:

$$
\phn[\Delta \omega] = \frac{k T F}{V_\mathrm{p}^2 \, \omega_0^2 C^2 R_\mathrm{p}} \cdot \left( \frac{\omega_0}{\Delta \omega} \right)^2
$$ {#eq-leeson-equation-rp}

so for a given tank capacitance $C$ the phase noise indeed *falls* with increasing $R_\mathrm{p}$, i.e., with a lower-loss tank. With this in mind, the design guidelines are:
...
3. Use an active circuit with low noise factor $F$ to minimize the noise contribution of the negative resistance $-R_\mathrm{amp}$. A good value to aim for is $F \approx 2$.
...
3. In RX, reciprocal mixing of the phase noise sidebands of the LO with strong adjacent channel signals creates in-band noise, which degrades the SNR and sensitivity of the RX.
...
::: {#nte-phnoise_division .callout-note}
## Phase Noise and Frequency Division
::: {.content-hidden when-format="revealjs"}
When using a frequency divider to generate a lower-frequency LO signal from a higher-frequency oscillator (e.g., to generate IQ phases), the phase noise of the divided signal is affected by the division ratio $N$ [@DaDalt_PHN]. Specifically, the phase noise of the divided signal improves by $20 \log_{10}(N)$ dB compared to the original oscillator phase noise. This means that if you divide the frequency by a factor of 2, the phase noise will be lowered by 6 dB!
:::

::: {.content-visible when-format="revealjs"}
Dividing the oscillator frequency by $N$ (e.g., for IQ generation) lowers the phase noise by $20 \log_{10}(N)$ dB, i.e., 6 dB for $N = 2$ [@DaDalt_PHN].
:::
...
$$
\sigma_\mathrm{a} = \frac{1}{2 \pi f_0} \sqrt{2 \int_{f_1}^{f_2} \phn[f] df}
$$ {#eq-jitter-phase-noise}

where $\sigma_\mathrm{a}$ is the absolute RMS jitter, $f_0$ is the carrier frequency, and the integral is taken over the frequency offset range from $f_1$ to $f_2$. This equation shows that higher phase noise levels lead to increased jitter, which can adversely affect the performance of digital communication systems by causing timing errors and signal integrity issues. Note that the phase noise $\phn[f]$ in @eq-jitter-phase-noise must be expressed in linear scale (not dBc/Hz) when using this equation.
...
$$
\begin{split}
\phn[3\,\text{MHz}] &= P_\mathrm{noise,max} - P_\mathrm{interferer} - 10 \log_{10}(B/\text{Hz}) \\
...
which consists of the series combination of a negative resistance $-R_\mathrm{amp}$ and a capacitance $C_\mathrm{amp}$. Note that the circuit in @fig-neg-r-single-ended does not show a ground symbol. In fact, any of the three nodes marked with blue numbers can be used as a reference node (ground), and this results in the following well-known oscillator topologies, summarized in @tbl-three-point-oscillators.

| Reference Node | Amplifier Configuration | Common Name |
|:---------------|:------------------------|:---------------------------------------------------------------------------|
| Node 1 (Gate)  | common-gate             | grounded-gate (also called common-base [Colpitts](https://en.wikipedia.org/wiki/Colpitts_oscillator)) |
| Node 2 (Source)| common-source           | [Pierce oscillator](https://en.wikipedia.org/wiki/Pierce_oscillator)        |
| Node 3 (Drain) | common-drain            | [Colpitts oscillator](https://en.wikipedia.org/wiki/Colpitts_oscillator)    |
: The three "three-point" oscillator topologies obtained by grounding one node of @fig-neg-r-single-ended {#tbl-three-point-oscillators}
...
::: {.content-hidden when-format="revealjs"}
The naming in @tbl-three-point-oscillators is not used consistently across the literature. The grounded-source case is universally called the Pierce oscillator, but *both* the grounded-gate and the grounded-drain variants are referred to as "Colpitts" by different authors (they differ only in which of the two capacitors provides the feedback), and some crystal-oscillator literature treats the grounded-drain circuit as just another implementation of the Pierce oscillator [@Vittoz_Crystal_Oscillators].
:::

::: {.content-visible when-format="revealjs"}
Naming in @tbl-three-point-oscillators varies: grounded source is always Pierce, but grounded gate and grounded drain are both called "Colpitts" [@Vittoz_Crystal_Oscillators].
:::

::: {.content-hidden when-format="revealjs"}
What is *not* a grounding variant is the **Clapp oscillator**: it is a Colpitts with an additional capacitor placed **in series with the inductor**, introduced to keep the transistor's parasitic capacitances from pulling the frequency [@Clapp.1948]. Interestingly, a quartz crystal oscillator gets this for free: as we will see in @fig-crystal-equivalent-circuit, the motional branch of a crystal is $L_\mathrm{m}$ in series with a very small $C_\mathrm{m}$, so $C_\mathrm{m}$ plays exactly the role of Clapp's series capacitor. This is the very reason why the crystal can be pulled only by a few ppm (see @eq-crystal-pulling), and thus why quartz oscillators are so stable.
:::

::: {.content-visible when-format="revealjs"}
The **Clapp oscillator** is a Colpitts with a capacitor **in series with the inductor** [@Clapp.1948]. A crystal has this built in: the small motional $C_\mathrm{m}$ (@fig-crystal-equivalent-circuit) limits pulling to a few ppm (@eq-crystal-pulling).
:::
:::
...
This means we can operate the quartz crystal as a high-Q inductor in an oscillator circuit, which results in the single-ended quartz crystal oscillator shown in @fig-crystal-oscillator, which is a very popular choice for high-performance crystal oscillators. For an in-depth treatment of this circuit, including its start-up behavior and amplitude regulation, we recommend [@Vittoz_Crystal_Oscillators]. Note that in a simple implementation the current-bias MOSFET can be replaced by an inverter stage biased in the linear region.
...
By putting the circuit in @fig-oscillator-lc-nmos on its head, we obtain a "PMOS core" oscillator, which is also widely used in integrated LC oscillators. This configuration is shown in @fig-oscillator-lc-pmos.
...
3. Use the gate-to-channel capacitance of a special MOSFET structure in accumulation mode, which can be achieved by using an NMOS situated in an n-well (in contrast to the usual operation in a p-well). Changing the gate-to-channel voltage changes the accumulation capacitance [@Andreani.2000].
...
3. Accumulation-mode MOSFET (NMOS in an n-well) [@Andreani.2000]
...
::: {.content-hidden when-format="revealjs"}
Data sheets and measurements state $K_\mathrm{VCO}$ as a *frequency* sensitivity in MHz/V or GHz/V, as defined above. All loop equations that follow (in @sec-oscillator-modeling and throughout @sec-pll) instead need the *angular* frequency sensitivity in rad/(s$\cdot$V), which is larger by a factor of $2 \pi$:
:::

::: {.content-visible when-format="revealjs"}
Data sheets use Hz/V; loop equations (@sec-oscillator-modeling, @sec-pll) need rad/(s$\cdot$V):
:::
