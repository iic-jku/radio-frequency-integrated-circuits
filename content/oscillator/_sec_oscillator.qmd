...
Note that no bias current sources are shown in @fig-oscillator-lc-cmos, but can be added in practice. However, they require some voltage headroom, which lowers the maximum voltage swing across the tank, which is negative for phase noise performance according to Leeson's equation in @eq-lesson-equation.
