...
In the following plot (@fig-gain-nonlinear-compression-expansion) an expansive behavior is also shown, where $\alpha_3$ has the same sign as $\alpha_1$, but $\alpha_5$ has the opposite sign, leading to gain expansion at some input power level, until the term with $\alpha_5$ becomes dominant and the gain is compressed again. This (partly) expansive behavior can be used to linearize the gain of a circuit block over a wider input power range, which is often desirable in RF power amplifiers (see @sec-poweramp-nonlinearity).
