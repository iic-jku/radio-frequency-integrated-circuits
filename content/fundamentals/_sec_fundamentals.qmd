...
In contrast, a system **with memory** will have an output at time $t$ which depends on the input at time $t$ and also on past inputs (e.g., at times $t - \Delta T$, $t - 2 \Delta T$, etc.). Such a system can additionally be **time-variant**, meaning its behavior changes over time, which leads to quite a few very interesting and important phenomena! Examples of systems with memory are filters, which have a frequency-dependent response, or power amplifiers with thermal memory effects.
:::
...
Some references also use similar definitions for gain expansion, where the output power is \qty{1}{\decibel} more than the linear slope.
:::

The following plot (@fig-gain-nonlinear-compression-expansion) also shows an expansive behavior. In contrast to the truncated Taylor-series model above, a closed-form saturating model is used here so that the output power correctly converges to the saturation power $P_\mathrm{sat}$ for large input levels. The model uses the amplitude characteristic

$$
A_\mathrm{out} = A_\mathrm{sat} \tanh \large[ u \cdot (1 + c \cdot u^2) \large]
$$

with the normalised amplitude $u = (G_0 P_\mathrm{in} / P_\mathrm{sat})^{-2}$ and an expansion coefficient $c > 1/3$. At small input powers the gain equals the small-signal gain $G_0$; at intermediate powers the gain expands above $G_0$ (because the cubic term in the argument of $\tanh$ adds extra gain before the $\tanh$ saturation takes effect); and at large input powers the output saturates at $P_\mathrm{sat}$ while the gain drops steeply. This (partly) expansive behavior can be used to linearize the gain of a circuit block over a wider input power range, which is often desirable in RF power amplifiers (see @sec-poweramp-nonlinearity).

{{< include /content/fundamentals/_fig_gain_nonlinear_compression_expansion.qmd >}}

### Multi-Tone Linearity

We now extend our investigations and apply two sinusoids with different frequencies and different amplitudes and see which signals we get at the output of the nonlinear block. The two-tone test and resulting third-order intermodulation products (IM3) are illustrated in @fig-im3-tones.
