...
$$
v_\text{out} = a_0 + a_1 v_\text{in} + a_2 v_\text{in}^2 + a_3 v_\text{in}^3 + a_4 v_\text{in}^4 + a_5 v_\text{in}^5 +\cdots
$$ {#eq-nonlinear-model}

Usually, the blocks under study will have higher-order nonlinear terms, but we often stop at 5rd order to keep things simple. For practical work, higher-order terms should be included if necessary.
...
$$
\begin{align}
	v_\text{out} & = \underbrace{a_0 + \frac{a_2 V_0^2}{2} + \frac{3 a_4 V_0^4}{8}}_{\text{DC (}\omega=0\text{)}}
	+ \underbrace{\left(a_1 V_0 + \frac{3 a_3 V_0^3}{4}+ \frac{5 a_5 V_0^5}{8}\right)\cos\omega_0 t}_{\text{fundamental }(\omega_0)} \nonumber\\
	&\quad + \underbrace{\left(\frac{a_2 V_0^2}{2}+\frac{a_4 V_0^4}{2}\right)\cos2\omega_0 t}_{\text{2nd harmonic }(2\omega_0)}
	+ \underbrace{\left(\frac{a_3 V_0^3}{4}+\frac{5 a_5 V_0^5}{16}\right)\cos3\omega_0 t}_{\text{3rd harmonic }(3\omega_0)} \nonumber\\
	&\quad + \underbrace{\frac{a_4 V_0^4}{8}\cos4\omega_0 t}_{\text{4th harmonic }(4\omega_0)}
	+ \underbrace{\frac{a_5 V_0^5}{16}\cos5\omega_0 t}_{\text{5th harmonic }(5\omega_0)}\cdots .
\end{align}
$$ {#eq-linearity-single}
The gain of the fundamental would be
$$
\begin{equation}\label{eq:non-linear-gain}
	G_v = \frac{v_\text{out}^{\omega_0}}{v_\text{in}^{\omega_0}} = \frac{a_1V_0 + \frac{3}{4} a_3 V_0^3 + \frac{5}{8} a_5 V_0^5}{V_0} =  a_1 + \frac{3}{4} a_3 V_0^2 + \frac{5}{8} a_5 V_0^4.
\end{equation}
...
- Odd-order nonlinearity ($\alpha_3$ and $\alpha_5$) can impact the gain of the fundamental term passing through the block. Usually $a_3$ and $a_5$ have the same sign, which is opposite to the sign of $a_1$. This implies that the gain stays by small input powers linear, then the terms with $a_3$ and $a_5$ starts to dominate at higher powers, leading to gain compression or saturation effect, until the amplifier reaches a level of maximum output power. In some cases, $a_3$ has the same sign of $a_1$ but $a_5$ the opposite one, which lead to gain expansion, meaning that the gain would have firstly an overshoot at some input power level, until the the term with $a_5$ is dominant and the gain is compressed again. These effects could be seen in Fig. @fig-gain-nonlinear-compression-expansion, where the output power in \unit{dBm} is plotted over input power in \unit{dBm}.
- Even- and odd-order nonlinearities create additional frequency components, so-called harmonics of the fundamental frequency. These harmonics are often unwanted, as they are far outside the wanted transmission frequency range, and need to be minimized, by either
  1. using a lowpass filter to filter these harmonics, or
...
<!--
Please see the figure attached t the pull request and add it here
@fig-gain-nonlinear-compression-expansion
-->
...
Some references uses also similar definitions for gain expansion, where the output power is \qty{1}{\decibel} more than the linear slope.
...
Flicker noise is usually modeled as a voltage noise source at the gate with a PSD of $K_f / (\Cox W L f)$ (in V$^2$/Hz), where $K_f$ is a process-dependent parameter, $\Cox$ is the oxide capacitance per unit area, $L$ and $W$ are the length and width of the MOSFET, and $f$ is the frequency. Note that we can lower the flicker noise by increasing the area of the MOSFET ($W L$), however, this increases the parasitic capacitances associated with the MOSFET, and this is often prohibitive for RF operation!
...
QAM is widely used in modern communication systems, as it allows transmitting more bits per symbol by combining amplitude and phase modulation. The form with four different symbols is called QPSK. Higher-order modulation like 16-QAM, for example, uses 16 different symbols, which can encode four bits per symbol (as $2^4 = 16$). Even higher-order QAM formats like 64-QAM (6 bits per symbol), 256-QAM (8 bits per symbol), 1024-QAM (10 bits per symbol), or 4096-QAM (12 bits per symbol) are also used in modern systems like WiFi or LTE.
...
For reference, the two-sided power spectral density $S(f)$ of a random binary sequence with equal probability of -1 and 1, using rectangular pulses with a duration of $T_\mathrm{b}$, is given by
...
A key property of OFDM is that the subcarriers are **orthogonal** to each other, which means that they do not interfere with each other, even if they overlap in frequency. This is achieved by choosing the subcarrier spacing $\Delta f$ such that it is equal to the symbol rate $1/T_\mathrm{sym}$, i.e., $\Delta f = 1/T_\mathrm{sym}$. This way, the integral of the product of two different subcarriers over one symbol period is zero, which means that they are orthogonal.
