...
#| echo: false
#| fig-cap: "Diagram of a PFD-CP PLL. Note that the capacitor $C_3$ has been added to reduce the CP switching ripple, and its presence makes the loop filter 3rd order."
...
with sd.Drawing(canvas='svg') as d:
    ...
    def draw_cp(pos):
        ...
        out_lf = d.here
        d.pop()
        rint = elm.Resistor().down().label(r'$R_\mathrm{int}$')
        cint = elm.Capacitor().down().label(r'$C_\mathrm{int}$')
        elm.Ground()
    ...
    (in1, in2, out_lf, cint) = draw_cp((d.unit*4,d.unit*1.5))
    ...
    d.push()
    elm.Capacitor().down().toy(cint.end).label(r'$C_3$', loc='top')
    elm.Ground()
    d.pop()
